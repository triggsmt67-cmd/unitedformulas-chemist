...
async function syncLibrarian() {
    ...
    try {
        ...
        guide += "## TECHNICAL GROUNDING (SDS & SAFETY)\n";
        groundingFiles.forEach(f => {
            const cleanName = f.name.split('grounding__')[1].replace('.txt', '').replace(/__/g, ' ');
            guide += `- ${cleanName} -> USE FILE: ${f.name}\n`;
        });

        guide += "\n## COMMERCE & METADATA\n";
        masterFiles.forEach(f => {
            const cleanName = f.name.split('sku_master__')[1].replace('.txt', '').replace(/__v1/g, '').replace(/-/g, ' ');
            guide += `- ${cleanName} -> USE FILE: ${f.name}\n`;
        });

        console.log("Generated Guide Preview:");
